...
class TrajaAccessor(object):
    ...
    def plot(self,x=None, y=None, n_coords: int = None, show_time=False, **kwargs):
        """Plot trajectory for single animal over period.

        Args:
          n_coords (int): Number of coordinates to plot
          x (str): Column name (Longitude)
          y (str): Column name (Latitude)
          **kwargs: 

        Returns:

        """
        # Assert x and y should be string 
        ...
        if x and y != None:
            ...
            coords = self._trj[['x', 'y']]  # TODO:Hard coded X and Y: Should be user defined coordinates/ column names in the dataframe 
    ...
    def xy(self):
        ...
        if self._has_cols(['x', 'y']):
            ...
            xy = self._trj[[self.x, self.y]].values  # User defined x and y column values from the dataframe
            return xy
